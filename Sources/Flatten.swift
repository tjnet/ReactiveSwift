...
extension Signal where Value: SignalProducerRepresentable, Error == Value.Error {
...
extension Signal where Value: SignalProducerRepresentable, Error == NoError {
	/// Flattens the inner producers sent upon `signal` (into a single signal of
	/// values), according to the semantics of the given strategy.
	///
	/// - note: If `signal` or an active inner producer fails, the returned
...
extension Signal where Value: SignalProducerRepresentable, Error == NoError, Value.Error == NoError {
	/// Flattens the inner producers sent upon `signal` (into a single signal of
	/// values), according to the semantics of the given strategy.
	///
	/// - warning: `interrupted` events on inner producers will be treated like
...
extension Signal where Value: SignalProducerRepresentable, Value.Error == NoError {
	/// Flattens the inner producers sent upon `signal` (into a single signal of
	/// values), according to the semantics of the given strategy.
	///
...
extension SignalProducer where Value: SignalProducerRepresentable, Error == Value.Error {
...
extension SignalProducer where Value: SignalProducerRepresentable, Error == NoError {
	/// Flattens the inner producers sent upon `producer` (into a single
	/// producer of values), according to the semantics of the given strategy.
	///
...
extension SignalProducer where Value: SignalProducerRepresentable, Error == NoError, Value.Error == NoError {
	/// Flattens the inner producers sent upon `producer` (into a single
	/// producer of values), according to the semantics of the given strategy.
	///
	/// - warning: `interrupted` events on inner producers will be treated like
...
extension SignalProducer where Value: SignalProducerRepresentable, Value.Error == NoError {
	/// Flattens the inner producers sent upon `signal` (into a single signal of
	/// values), according to the semantics of the given strategy.
	///
	/// - note: If `signal` fails, the returned signal will forward that failure
...
extension Signal {
	...
	public func flatMap<Inner: SignalProducerRepresentable>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> Signal<Inner.Value, Error> where Inner.Error == Error {
	...
	///   - transform: A closure that takes a value emitted by `self` and
	///                returns a signal producer with transformed value.
	public func flatMap<Inner: SignalProducerRepresentable>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> Signal<Inner.Value, Error> where Inner.Error == NoError {
		return map(transform).flatten(strategy)
	}
}
...
extension Signal where Error == NoError {
	...
	///   - transform: A closure that takes a value emitted by `self` and
	///                returns a signal producer with transformed value.
	public func flatMap<Inner: SignalProducerRepresentable>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> Signal<Inner.Value, Inner.Error> {
		return map(transform).flatten(strategy)
	}
	...
	///   - transform: A closure that takes a value emitted by `self` and
	///                returns a signal producer with transformed value.
	public func flatMap<Inner: SignalProducerRepresentable>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> Signal<Inner.Value, NoError> where Inner.Error == NoError {
		return map(transform).flatten(strategy)
	}
}
...
extension SignalProducer {
	...
	///   - transform: A closure that takes a value emitted by `self` and
	///                returns a signal producer with transformed value.
	public func flatMap<Inner: SignalProducerRepresentable>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Error> where Inner.Error == Error {
		return map(transform).flatten(strategy)
	}
	...
	///   - transform: A closure that takes a value emitted by `self` and
	///                returns a signal producer with transformed value.
	public func flatMap<Inner: SignalProducerRepresentable>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Error> where Inner.Error == NoError {
		return map(transform).flatten(strategy)
	}
}
...
extension SignalProducer where Error == NoError {
	...
	///   - transform: A closure that takes a value emitted by `self` and
	///                returns a signal producer with transformed value.
	public func flatMap<Inner: SignalProducerRepresentable>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Inner.Error> {
		return map(transform).flatten(strategy)
	}
}
